...
# Přečteme templates.py
with open("templates.py", "r", encoding="utf-8") as f:
    tpl_code = f.read()

# Úplně nová a čistá, na 100% kompletní definice DETAIL_UI
FULL_DETAIL_UI = """<div id="live-sync-container" data-tid="{{ tournament.id }}">
...
# Pomocí regulárního výrazu najdeme konec definice MATCH_MACRO a začátek definice HOF_HTML
# Všechno mezi tím natvrdo nahradíme
tpl_code = re.sub(r'(MATCH_MACRO \+ """<div id="live-sync-container".*?<\/script>""")', f'MATCH_MACRO + """{FULL_DETAIL_UI}"""', tpl_code, flags=re.DOTALL)
